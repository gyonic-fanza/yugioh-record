...
import {analyze,battlesCsv,periodBounds} from '../js/analysisService.js';
...
const html=renderAnalysis(data,{period:`period:${period.id}`});assert(html.includes('月別MATCH推移'));assert(html.includes('デッキバージョン比較'));assert(html.includes('少数（5戦未満）'));assert(!html.includes('<script>'));assert(renderAnalysis(data,{period:'custom',from:'2026-10-01',to:'2026-09-01'}).includes('開始日が終了日より後')); 
