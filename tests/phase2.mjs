...
const sep=analyze(data,{key:'custom',from:'2026-09-01',to:'2026-09-30'});assert.equal(sep.match.total,1);assert.equal(sep.game.total,2);assert.equal(sep.single.total,1);assert.equal(analyze(data,{key:'custom',from:'2026-07-01',to:'2026-08-31'}).singleMonths.length,0);
...
const html=renderAnalysis(data,{period:`period:${period.id}`});assert(html.includes('月別MATCH推移'));assert(html.includes('月別SINGLE推移'));assert(html.includes('SINGLE 2026-09: 1戦 1勝 0敗 0分'));assert(html.includes('id="analysis-event-name"'));assert(renderAnalysis(data,{eventName:'フリー'}).includes('data-event-suggestion="フリー対戦"'));assert(html.includes('デッキバージョン比較'));assert(html.includes('少数（5戦未満）'));assert(!html.includes('<script>'));assert(renderAnalysis(data,{period:'custom',from:'2026-10-01',to:'2026-09-01'}).includes('開始日が終了日より後')); 
